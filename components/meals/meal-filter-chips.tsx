...
import { pillTabsListClass, pillTabsTriggerClass } from "@/lib/layout";
...
export function MealFilterTabsList({ counts }: { counts: Record<TMealFilter, number> }) {
  return (
    <div className="w-full min-w-0 overflow-x-auto overscroll-x-contain scroll-smooth [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      <TabsList className={cn(pillTabsListClass, "w-max max-w-none")}>
        {MEAL_FILTERS.map((type) => (
          <TabsTrigger
            key={type}
            value={type}
            className={cn(pillTabsTriggerClass, "group gap-1.5", MEAL_TAB[type].trigger)}
          >
            <span
              aria-hidden
              className={cn(
